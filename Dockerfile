...
# and `pip install calculoastronomico` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "calculoastronomico", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n calculoastronomico -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "calculoastronomico", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "calculoastronomico", "YOUR_CONSOLE_SCRIPT"]
